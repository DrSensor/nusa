...
*/ export default (Class, callback, { controller }) => {
  dedupeUpdate?.();
  const [access, iterate, restore] =
    /** @type _Pipeline */ (flow.get(controller));
...
*/ export class IterateController {
  /** @type currentIndex | undefined */ #index;
  #access = /** @type ToA */ ({});

  #stopAt = Infinity;
  /** @type number[] */ #skipAt = [];
  /** @type Map<number, _StructFrom<ToA>> */ #backup = new Map(); // TODO: investigate if V8 Map<number, any> optimized into SparseSet[sparse|indices: number[], dense|values: any[]]
  ...
  /** @type _SoA */ proxy = new Proxy({}, {
    get: /** @param accessor{string} */ (_, accessor) =>
      /** @type _SoA */ (currentAccess)[accessor] =
        /** @type Record<string, _AccessorBinder> */ (
          memberAccess
        )[accessor].databank_,
  });
